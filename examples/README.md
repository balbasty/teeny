...
| `cholesky_solve.cpp` | Small SPD Cholesky factor + solve on a matrix with **per-dimension compile-time strides** (`wrap(..., strides<S...>{})`) and stack-owned work tensors that are exactly `sizeof` their data. |
...
| `pushpull_adjoint.cpp` | the flagship fastfields kernel: rank-generic spline **pull** (gather) and **push** (scatter, atomic on host and device), validated by the adjoint identity `<Px,y>==<x,Pᵀy>` across interpolation orders 0–3 and 4 boundary conditions. |
