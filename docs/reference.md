...
| `dispatch_index<Idx2=int32_t>(v, f)` | `void` | narrow a fixed-rank view `v` — **or a whole `anyrank` carrier** — to `Idx2` offsets when its span fits, else keep it, then call `f`; instantiates `f` for both widths |
