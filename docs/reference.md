...
| `t.index_fits<Idx2>()` (free: `index_fits<Idx2>(t)`) | `bool` | do all element offsets fit `Idx2`? Signed reach `Σ_{s>0}(e−1)s` / `Σ_{s<0}(e−1)s` ⊆ `Idx2` (handles negative-stride/flipped views; broadcast stride 0 adds 0). `Idx2` may be **any integral type up to 64 bits**, signed or unsigned (`uint64_t`/`size_t` included): the positive and the negative reach accumulate in separate unsigned/signed 64-bit domains, so neither comparison can wrap. The view's OWN index type may equally be unsigned 64-bit (`shape_as<uint64_t, …>`): each extent/stride is measured in the type it is stored in, so a value above `int64_t`'s range is counted for what it is instead of being mis-read as negative. It answers a question about reachable **offsets**, not about extent *values* — an axis with no reach (stride 0) whose extent alone exceeds `Idx2` still passes, and `reindex` would then truncate that extent |
