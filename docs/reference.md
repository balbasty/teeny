...
| `at.index_fits<Idx2>()` (free: `index_fits<Idx2>(at)`) | `bool` | does every offset reachable through the **carrier** fit `Idx2`? Same signed-reach contract as the view's `index_fits` (negative-stride safe; broadcast stride 0 adds 0) |
