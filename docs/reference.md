...
| `t.reindex<Idx2>()` (free: `reindex<Idx2>(t)`) | → view | no-copy, **layout-preserving** retype of the offset **index width** to `Idx2`: same pointer + layout kind, extents & dynamic strides narrowed to `Idx2` (a `strides<...>` literal pack unchanged). Narrows a boundary view to `shape32` — halves the by-value footprint, 32-bit device offset math. Orthogonal to `recast` (which staticizes the extent *values*); they compose. Debug-checks `index_fits<Idx2>()` — every offset *and* every dynamic extent value — and is UB if the caller lies; a *static* extent too large for `Idx2` is a compile error |
| `t.index_fits<Idx2>()` (free: `index_fits<Idx2>(t)`) | `bool` | can this view be re-expressed with index type `Idx2`? Two conditions, one predicate — the precondition `reindex<Idx2>()` checks. **(1) offsets**: signed reach `Σ_{s>0}(e−1)s` / `Σ_{s<0}(e−1)s` ⊆ `Idx2` (handles negative-stride/flipped views; broadcast stride 0 adds 0). **(2) extent values**: every axis's size is representable in `Idx2` — `reindex` narrows the *shape* too, and a narrowed extent is the loop bound everywhere downstream (`numel()`, `size_front`, the peel odometer, every kernel loop), so a truncated one silently does the wrong amount of work. The two come apart on a broadcast (stride-0) axis, which reaches nothing however large its extent. `Idx2` may be **any integral type up to 64 bits**, signed or unsigned (`uint64_t`/`size_t` included): the positive and the negative reach accumulate in separate unsigned/signed 64-bit domains, so neither comparison can wrap. The view's OWN index type may equally be unsigned 64-bit (`shape_as<uint64_t, …>`): each extent/stride is measured in the type it is stored in, so a value above `int64_t`'s range is counted for what it is instead of being mis-read as negative. A *static* extent too large for `Idx2` is a compile error from `reindex` itself, so this runtime query is about the dynamic ones |
...
| `dispatch_rank<narrow_index>(at, f)` | `bool` | ...and narrow each fixed cell's offset width to `int32` when `index_fits` says the narrowing is lossless (rank outer, width inner; the leaf doubles). Default `Narrow=false` is the plain dispatch |
| `dispatch_index<Idx2=int32_t>(v, f)` | `void` | narrow a fixed-rank view `v` — **or a whole `anyrank` carrier** — to `Idx2` offsets when its span fits, else keep it, then call `f`; instantiates `f` for both widths. `Idx2` need **not** be able to hold `v`'s *compile-time* sizes: one too large for it rules the narrow arm out at compile time, so only the wide arm is instantiated and the call still builds — generic code can hand it any shape. A direct `v.reindex<Idx2>()` on such a shape stays a compile error |
| `at.index_fits<Idx2>()` (free: `index_fits<Idx2>(at)`) | `bool` | can the **carrier** be re-expressed with index type `Idx2` — every reachable offset *and* every axis size? Same contract as the view's `index_fits` (negative-stride safe; broadcast stride 0 adds no reach, but its size is still checked, since `reindex` narrows the carrier's shape too and that shape is the loop bound for every cell peeled off it). `Idx2` may be **any integral type up to 64 bits**, signed or unsigned (`uint64_t`/`size_t` included): the positive and the negative reach accumulate in separate unsigned/signed 64-bit domains, so neither comparison can wrap. The carrier's own offset type is deduced from the shape/stride arrays you hand `as_anyrank`, so it may itself be `uint64_t`/`size_t`; each value is measured in that type, never cast down to a signed one first |
| `at.reindex<Idx2[, MaxRank]>()` (free: `reindex<Idx2>(at)`) | narrowed `anyrank` | the same carrier — same data pointer, memory space and static `anyshape` head/tail geometry — with `ndim` and the runtime shape/strides copied into an inline `Idx2` store, so **every cell it hands out is `Idx2`-indexed**. Narrow **once, host-side, before a launch** and keep the batch idiom (which `dispatch_rank` would collapse and per-cell `dispatch_index` cannot reach from a kernel). Halves the by-value meta store (`MaxRank × 2 × 8 B` → `× 4 B`) and runs cell offsets in 32-bit. Always an inline (`copy_meta`) store — narrowing copies; `MaxRank` defaults to the source's capacity. Debug-checks `index_fits<Idx2>()` — every offset *and* every dynamic extent value — and is UB if the caller lies; a *static* `anyshape` head/tail dim too large for `Idx2` is a compile error. Import width is never narrowed for you (`from_dlpack` keeps DLPack's `int64_t`) |
...
| `at.peel_front(Int<N>())` / `at.peel_front_at(i, Int<N>())` — template form `at.peel_front<N>()` / `at.peel_front_at<N>(i)` | range / view | batch idiom: keep the last `\|N\|` dims static, peel the rest (one kernel per `\|N\|`). The keep-count is a single compile-time selector, so it also takes the `Int<N>()` **value form** — deduced, so a carrier whose type is a template parameter needs no `.template`. A `shape<…>{}` in that same second position selects the fused recast below, never the keep-count |
...
| `at.size_front(Int<N>())` — template form `at.size_front<N>()` | → offset | flattened batch count `peel_front` yields (product of the peeled leading extents), no range built; `N < 0`. Same `Int<N>()` value form as `peel_front` |
| `dispatch_value<Vs...>(v, f)` | `bool` | call `f(Int<k>{})` for the matching candidate `k == v` |
| `dispatch_values(f, candidates<Vs...>(v), …)` | `bool` | the **product** form: one candidate list per parameter, `f` called with one `Int<k>{}` per list (in list order). Exactly the nested `dispatch_value` pyramid — one comparison per parameter, `f` instantiated once per **combination** of candidates — written as one call, with the instantiation budget visible in one place. A value outside its own list doesn't fire (`f` not called, returns `false`); `true` means every value matched and `f` ran. The runtime value may be an integer **or an enum** (no hand `static_cast`). `f` comes **first**: the candidate lists are variadic |
...
| `at.peel_front_at(i, Int<-Sr>())` / `at.peel_front_at<-Sr>(i)` | `dyn_tensor<T, offset_t, Sr, Space>` | one batch cell (rank `Sr`); `.recast<shape<-1,c,c>>()` recovers static inner dims |
