...
| `t.index_fits<Idx2>()` (free: `index_fits<Idx2>(t)`) | `bool` | do all element offsets fit `Idx2`? Signed reach `Σ_{s>0}(e−1)s` / `Σ_{s<0}(e−1)s` ⊆ `Idx2` (handles negative-stride/flipped views; broadcast stride 0 adds 0). `Idx2` must be signed (any width up to 64 bits) or unsigned up to 32 bits — a wider unsigned target (`uint64_t`, `size_t` on LP64, etc.) is a compile error, not a runtime `false` |
...
| `at.index_fits<Idx2>()` (free: `index_fits<Idx2>(at)`) | `bool` | does every offset reachable through the **carrier** fit `Idx2`? Same signed-reach contract as the view's `index_fits` (negative-stride safe; broadcast stride 0 adds 0). `Idx2` must be signed (any width up to 64 bits) or unsigned up to 32 bits — a wider unsigned target (`uint64_t`, `size_t` on LP64, etc.) is a compile error, not a runtime `false` |
