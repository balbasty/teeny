...
int main()
{
    ...
    // ---- #38: memory-space tag. Default is host (storage::view); as_anyrank<Space>
    ...
    static_assert(decltype(at)::space == storage::view, "default carrier is host");
    static_assert(decltype(at.fixed<3>())::ownership == storage::view, "host fixed -> view");
    auto dev = as_anyrank<storage::gpu_view>(buf, shape, stride, 3);   // borrowed device ptr
    static_assert(decltype(dev)::is_device, "as_anyrank<gpu_view> is device");
    static_assert(decltype(dev.fixed<3>())::ownership == storage::gpu_view, "device fixed -> gpu_view");
