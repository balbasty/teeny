...
int main()
{
    ...
    {
        auto a = t(1, all);
        auto b = t.uslice(1, all);
        static_assert(cs::is_same<decltype(a), decltype(b)>::value, "uslice(all) type == slice type");
        if (a.extent(0) != b.extent(0)) return 9;
        for (long j = 0; j < 6; ++j) if (a(j) != b(j))   return 10;
    }
    {
        auto a = t(slice(1, 3), slice(none, 4));
        auto b = t.uslice(slice(1, 3), slice(none, 4));
        static_assert(cs::is_same<decltype(a), decltype(b)>::value, "uslice runtime-bound type == slice type");
    ...
    // a STATIC negative bound folds the SAME under uslice as under operator()
    ...
    // which the forward clamp collapses to an EMPTY axis. No UB either way.
    {
        ...
        auto unw = t.uslice(0, slice(1, (long)-1)); // [1 .. -1) -> empty (no wrap)
