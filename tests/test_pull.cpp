...
int main()
{
    ...
    {
        auto inp = wrap(buf, shape<5,6>{});     // row-major, strides (6,1)
    ...
    {
        auto inp = wrap(buf, shape<4,5,6>{});   // strides (30,6,1)
    ...
    {
        using E = shape<cs::dynamic_extent, cs::dynamic_extent, cs::dynamic_extent>;
