...
int main() {
    ...
    // with a fitting extent, also answers `true`. The nearest existing case ((14)(c)
    // above) hits reach -128 but is rejected on the EXTENT half of the predicate (129
    // doesn't fit int8_t) — it never exercises the reach-equality half. Here the extent
    // (2) trivially fits int64_t, so only the reach-equality half is in play: reach =
    // (2-1)*INT64_MIN == INT64_MIN exactly. (`INT64_MIN` is spelled
    // `-9223372036854775807LL - 1` — the positive magnitude 9223372036854775808 doesn't
    // fit `long long`, so the direct literal would misparse; this form is exact.)
